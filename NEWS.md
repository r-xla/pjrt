...
* pjrt now exposes an **interface for downstream packages**, reached with
  `LinkingTo: pjrt`. It comes in two halves: `inst/include/pjrt/api.h`, a
  hand-written C interface for the metadata reads a caller runs on a hot path,
  and `pjrt_RcppExports.h` (namespace `pjrt`), generated by Rcpp attributes,
  for the calls that allocate or execute -- those marshal through a SEXP, which
  costs an allocation per call and in exchange propagates R errors, interrupts
  and longjumps into the caller as C++ exceptions. It lets another
