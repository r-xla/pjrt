...
2. **One spec, three parts, full arc (A → B → C).** The user wants the native
   dispatcher to eventually cover *everything* (design-doc §6). Part A (native
   static) is self-contained and approved; Part B moves the pytree module to pjrt
   (opaque Node); Part C generalizes the dispatcher to all backends and cases,
...
- **pjrt C++:** `impl_dispatch_create(int capacity, SEXP miss_fn, SEXP static_names)`.
  ...
  jits). Stored on `Dispatcher` as `std::unordered_set<std::string> static_names_`.
- **pjrt R:** `pjrt_dispatcher(capacity, compile, static = character())`.
- **anvl:** `jit_xla_impl` passes `static = static` when creating the dispatcher.
  The compile callback `jit_xla_compile_cb(f, static, donate)` already handles
  static via `jit_prepare_args(args, static, …)` — **no change**.
- `pjrt_dispatch(dispatcher, args)` and `impl_dispatch_run(handle, args)`
...
- The dispatcher only `set()`s on a miss (always a fresh key → `push_front`,
...
Goal: the native dispatcher becomes the **single** cache + dispatch path for every
...
### C.1 Cases the dispatcher must cover
...
- **Device / `device_arg` moves**: the target device is part of the key; inputs
  ...
  (static) arg. The dispatcher is created **unconditionally** (not only when
  `device = NULL`).
...
The dispatcher is created per (function, backend, device policy) — a jit fixes its
...
lazily-created dispatcher). The cache entry becomes a small variant:
...
- **quickr via dispatcher:** a quickr jit populates the native cache
...
- **After C:** the dispatcher covers all of the above; there is **no R fallback**.
