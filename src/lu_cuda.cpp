...
//
// cuSOLVER's getrf places ipiv (and devInfo) directly in device memory --
// the pivots output buffer XLA hands us is already on-device, so we pass
// it through with no extra D2D copy.
#include <Rcpp.h>
...
static Error lu_cuda_impl(void *stream, ScratchAllocator &scratch,
  ...
  // getrf overwrites A in place. Copy input -> LU output buffer first, then
  // factorise the LU buffer so we never touch the input.
