...
public:
  ...
  execute(std::vector<PJRTBuffer *> input);
...
public:
  ...
  compile(const PJRTProgram &program, PJRTCompileOptions &compile_options);
  std::unique_ptr<PJRTBuffer>
  buffer_from_host(void *data, const std::optional<std::vector<int64_t>> &dims,
