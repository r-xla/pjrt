...
std::string PJRTMemory::debug_string() {
  ...
  check_err(this->api.get(), this->api->PJRT_Memory_DebugString(&args));
...
int PJRTMemory::id() {
  ...
  args.memory = this->memory;
  check_err(this->api.get(), this->api->PJRT_Memory_Id(&args));
  return args.id;
}
...
std::string PJRTMemory::kind() {
  ...
  args.memory = this->memory;
  check_err(this->api.get(), this->api->PJRT_Memory_Kind(&args));
...
std::string PJRTMemory::to_string() {
  ...
  args.memory = this->memory;
  check_err(this->api.get(), this->api->PJRT_Memory_ToString(&args));
