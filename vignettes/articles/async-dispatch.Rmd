...
A key feature of the async API is **smart input handling**. When you pass async values as inputs:
...
- **Async execution results** (`pjrt_async_value`): Must wait for execution to complete before the buffer is valid.

This enables true pipelining where host-to-device transfers overlap with execution:
