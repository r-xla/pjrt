...
- **Async transfers** (`pjrt_async_transfer`): The buffer is used immediately - PJRT handles the dependency internally. R returns immediately without blocking.
- **Async execution results** (`pjrt_async_value`): Can be passed to `as_array_async()` or used as inputs to other executions - PJRT handles the dependency.
