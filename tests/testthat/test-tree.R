# The pytree module (src/tree.h, src/tree.cpp, R/tree.R). These tests were
...
test_that("tree_kind and child_kinds", {
  expect_equal(tree_kind(build_tree(1)), "leaf")
  expect_equal(tree_kind(build_tree(list(1))), "list")
  expect_equal(tree_kind(build_tree(NULL)), "null")
  expect_equal(
    child_kinds(build_tree(list(1, list(2), NULL))),
    c("leaf", "list", "null")
  )
  expect_error(child_kinds(build_tree(1)), "list node")
})

test_that("tree_names, child_sizes, and flat_names", {
  ...
  expect_equal(child_sizes(tree), c(1L, 2L, 0L))
  expect_equal(flat_names(tree), c("a", "b", "b"))
  expect_equal(flat_names(build_tree(list(1, 2))), c("", ""))
})

test_that("mask_from_names marks all leaves under matching top-level names", {
  tree <- build_tree(list(a = 1, b = list(2, list(3)), c = NULL, d = 4))
  expect_equal(mask_from_names(tree, "b"), c(FALSE, TRUE, TRUE, FALSE))
  expect_equal(mask_from_names(tree, c("a", "d")), c(TRUE, FALSE, FALSE, TRUE))
  # empty names -> all-FALSE; unknown names ignored
  expect_equal(mask_from_names(tree, character()), rep(FALSE, 4))
  expect_equal(mask_from_names(tree, "zzz"), rep(FALSE, 4))
  # an unnamed tree marks nothing
  expect_equal(mask_from_names(build_tree(list(1, 2)), "a"), c(FALSE, FALSE))
})

test_that("filter_by_names keeps children in tree order and reindexes", {
  x <- list(a = 1, b = 2, c = 3)
  tree <- build_tree(x)
  sub <- filter_by_names(tree, c("a", "c"))
  expect_equal(tree_size(sub), 2L)
  expect_equal(unflatten(sub, x[c("a", "c")]), x[c("a", "c")])

  # selection order does not matter; tree order wins
  sub2 <- filter_by_names(tree, c("c", "a"))
  expect_true(tree_equal(sub, sub2))

  # keeping everything preserves the structure
  all_kept <- filter_by_names(tree, c("a", "b", "c"))
  expect_true(tree_equal(all_kept, tree))

  # nested children reindex contiguously
  y <- list(a = list(1, 2), b = 3, c = list(d = 4))
  sub3 <- filter_by_names(build_tree(y), c("a", "c"))
  ...
  expect_error(filter_by_names(build_tree(list(1, 2)), "a"), "names")
})
...
test_that("flatten_fun", {
  ...
  f_flat <- do.call(flatten_fun, c(list(f), args))
...
test_that("property: random nested structures round-trip", {
  ...
  # R-side reference implementations of flatten and mask_from_names
  ...
  for (rep in 1:50) {
    ...
    # mask_from_names agrees with the reference expansion
    if (is.list(x) && !is.object(x) && length(x) > 0) {
      ...
      expect_identical(mask_from_names(tree, marked), ref_mask)
    }
  }
})
