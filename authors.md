# Authors and Citation

## Authors

- **Daniel Falbel**. Author, maintainer.
  [](https://orcid.org/https://orcid.org/0009-0006-0143-2392)

- **Sebastian Fischer**. Author.
  [](https://orcid.org/https://orcid.org/0000-0002-9609-3197)

## Citation

Source:
[`DESCRIPTION`](https://github.com/r-xla/pjrt/blob/feat-format-buffer/DESCRIPTION)

Falbel D, Fischer S (2025). *pjrt: R Interface to PJRT*. R package
version 0.0.0.9000, <https://r-xla.github.io/pjrt/>.

    @Manual{,
      title = {pjrt: R Interface to PJRT},
      author = {Daniel Falbel and Sebastian Fischer},
      year = {2025},
      note = {R package version 0.0.0.9000},
      url = {https://r-xla.github.io/pjrt/},
    }
